<?php
    ...
    function generateSignature($payment_request_obj, $secret_key) {
        ...
        $signature = arraryToString($json_array);

        //5) Generate hashed signature by using HMAC-SHA256 with merchant sercret key 
        $hashed_signature = hashSignature($signature, $secret_key);
        
        return $hashed_signature;
    }

    //Method for generate signature by JSON string
    function generateSignatureByJSONString($raw_json_string, $secret_key) {
    ...
    function validateSignature($encoded_response, $secret_key) {
        ...
        $json_array = jsonToArray($raw_response);
        ...
        $self_hashed_signature = generateSignatureByJSONString($raw_json_after_removed_signature, $secret_key); 
    ...
    function multipleToSingleDimensionalArray($array) {
        return iterator_to_array(new RecursiveIteratorIterator(new RecursiveArrayIterator($array)), 0);
    }
    
    function convertToStringArray($array) {
        return array_map('strval', $array);
    }
    
    function sortArray(&$array) {

        //orting setting equivalent to C# sort() 
        sort($array, SORT_STRING | SORT_FLAG_CASE);
    }
    
    function arraryToString($array) {
        return implode($array);
    }
    
    function hashSignature($signature, $secret_key) {
        return hash_hmac('sha256', $signature, $secret_key);
    }

    //Method for API request
    function requestAPI($api_env, $payment_request_obj) {
        ...
        return $encoded_response = doAPIRequest($api_env, $payment_request_json);
    }

    function doAPIRequest($api_env, $raw_json_request) {
    ...
    function parseAPIResponse($encoded_response) {
    
        //Decode encoded reponse to JSON string with Base64
        $raw_response = base64_decode($encoded_response);

        //Convert JSON string to object
        $json_object = jsonToObject($raw_response);
